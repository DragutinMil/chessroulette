...
import { Chapter, ChapterState, DailyLessonActivityState } from './types';
...
export const findLoadedChapter = (
  activityState: DailyLessonActivityState['activityState']
): Chapter | undefined =>
...
export const reducer: MovexReducer<ActivityState, ActivityActions> = (
  ...
  if (prev.activityType !== 'dailylesson') {
    return prev;
  }

  if (action.type === 'loadedChapter:addDailyLessonMove') {
    try {
      ...
      const { move, forceTurn, lesson } = action.payload;

      const fenBoard = new ChessFENBoard(prevChapter.displayFen);
      if (!fenBoard.piece(move.from)) {
        console.error('No piece at', move.from);
        return prev;
      }

      const nextMove = fenBoard.move(move);

      // Koraci tipa 'teach' su uzastopni potezi iste boje, pa posle poteza
      // vracamo potez korisniku - inace bi chess.js odbio sledeci potez.
      let nextFen = fenBoard.fen;
      if (forceTurn) {
  ...
  if (action.type === 'loadedChapter:setDailyLesson') {
    return {
      ...prev,
      activityState: {
        ...prev.activityState,
