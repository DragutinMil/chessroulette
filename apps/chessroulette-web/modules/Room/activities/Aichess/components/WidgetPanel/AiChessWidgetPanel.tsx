...
export const AiChessWidgetPanel = React.forwardRef<TabsRef, Props>(
  (
    ...
    const onTabChange = useCallback(
      ...
      const isMyTurn = currentChapterState.displayFen.split(' ')[1] ===  currentChapterState.orientation;
      ...
      const addQuestion = async (question: string) => {
          ...
          const data = await SendQuestion(question, currentChapterState, stockfishMovesInfo);
          if(data){
             setPulseDot(false)
          }
    ...
    useEffect(() => {
      if (
        currentChapterState.chessAiMode.badMoves > 0 &&
        isMyTurn
      ) {
        onMessage({
   ...
   const stockfishInfo = (m: any) => {
    ...
    const puzzles = async () => {
     ...
     //FIRST MOVE
        ...
        const play = async () => {
          addChessAi({ moves: [], movesCount:0 ,badMoves:0,goodMoves:0,orientationChange:false, mode:'play'});
...
//         }
    ...
    return (
      <div className="  flex flex-col flex-1 min-h-0 rounded-lg shadow-2xl flex-1 flex min-h-0 ">
        <StockFishEngineAI 
        fen={currentChapterState.displayFen}  puzzleMode={puzzleMode} playMode={playMode}
        stockfishInfo={stockfishInfo}
        isMyTurn={isMyTurn} engineMove={engineMove}   />
          ...
          tabs={[
            {
              ...
              renderContent: () => (
                <div className="flex flex-col flex-1 gap-2 min-h-0">
                  <Conversation currentChapterState={currentChapterState} pulseDot={pulseDot} />
                  ...
                  >
                    { (currentChapterState.chessAiMode.mode=='puzzle'  || currentChapterState.chessAiMode.mode=='openings') && currentChapterState.notation.history.length>1  ?(
