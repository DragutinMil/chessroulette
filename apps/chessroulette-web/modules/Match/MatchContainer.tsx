...
import { usePlayerAbandonDetection } from '@app/hooks/usePlayerAbandonDetection';
...
const MatchContent: React.FC<MatchContentProps> = ({ inviteLink, userId, match, dispatch, ...boardProps }) => {
  // Hook pozvan unutar MatchProvider konteksta
  usePlayerAbandonDetection();
  ...
  const otherPlayerChatEnabled = useMemo(() => {
    const otherPlayer =
      match.challenger.id === userId ? match.challengee : match.challenger;
    return otherPlayer.isChatEnabled !== false;
  }, [match.challenger, match.challengee, userId]);
            ...
            {activeWidget === 'camera' ? (
              ...
              <ChatWidget
                ...
                onSendMessage={(content) => {
                  dispatch((masterContext: { requestAt: () => any; }) => ({
