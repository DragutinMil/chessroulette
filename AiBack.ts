  ...
  const tools = [
    {
      ...
      description:
        "Retrieves chess puzzle 'FEN' string and solution array('solution') for the given puzzle theme param",
      parameters: {
        type: 'object',
        properties: {
          theme: {
  ...
  if (out && out.type == 'function_call') {
    ...
    switch (out.name) {
      case 'get_random_puzzle':
        console.log(
          `Calling ${out.name} with arguments: ${JSON.stringify(args)}`
        );
        try {
          fn_resp = await db.queryByName(`GET_puzzle_random_by_theme`, {
            vars: { ...req.vars, ...args },
            auth: req.auth,
          });
        } catch (e) {
